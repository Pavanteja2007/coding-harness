...
def test_fix_e2e_offline(smoke_repo, home, logs_root, capsys, monkeypatch):
    ...
    assert "task fix-" in out and "success" in out
    assert "target test:" in out and "PASS" in out
...
def test_fix_issue_from_file(
    smoke_repo, home, logs_root, capsys, monkeypatch, tmp_path
):
    import harness.deps as hdeps
    ...
    issue_file.write_text(
        "mean() in mathutil.py returns the sum; make it the mean", encoding="utf-8"
    )
    rc = main(
        [
            "fix",
...
def test_fix_reports_clean_model_error(smoke_repo, home, capsys, monkeypatch):
    ...
    assert rc == 1
    assert "task fix-" in captured.out and "error" in captured.out
...
def test_scheduler_stub_fanout(home, capsys):
    ...
    tasks = [
        Task(task_id=f"t{i}", repo_path=".", issue_text="x", config={})
        for i in range(6)
    ]
...
def test_scheduler_stub_crash_isolated(home, capsys):
    ...
    tasks = [
        Task(task_id=n, repo_path=".", issue_text="x", config={})
