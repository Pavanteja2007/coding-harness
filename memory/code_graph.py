...
Indexes a Python repository's structure — functions, classes, methods,
...
Query surface (see CodeGraph.query and QUERY_HELP):
    ...
    callees <name>           — what this function/method calls
...
Call-edge caveat: Python is dynamic, so call edges are best-effort —
...
Assumes: Python-only repos (project tech lock — Phase 1 scope);
tree-sitter-python is the only grammar wired in.
"""
...
SKIP_DIR_NAMES = {
    ...
    ".venv", "venv", "env", ".git", ".hg", ".svn", ".idea", ".vscode",
    "node_modules", ".harness", "logs", "build", "dist", "docs", "site",
}
            ...
            if node.type == "import_statement":
                ...
                for ch in node.children:
                    ...
                    if ch.type == "identifier":
                        ...
                        for sub in ch.children:
                            if sub is not None and sub.type in ("dotted_name", "identifier"):
...
class CodeGraphBuilder:
    ...
    def build(self) -> Graph:
        """Walk the repo, parse every .py file, build and link the graph.

        Assumes a Python repo; non-.py files are ignored entirely. Parse
        errors never abort the build — the file is skipped and counted.
        """
        ...
        for rel, source in self._iter_python_files():
            try:
            ...
            graph.nodes[f"file:{rel}"] = NodeInfo(
                kind="file", name=rel, qualified=rel,
                file=rel, line=0, end_line=0,
            )
            for info in idx.symbols:
                prefix = "method:" if info.kind == "method" else (
                    "class:" if info.kind == "class" else "func:")
        ...
        for mod, rel in modules.items():
            graph.nodes[f"module:{mod}"] = NodeInfo(
                kind="module", name=mod, qualified=mod,
                file=rel, line=0, end_line=0,
            )
    ...
    def _iter_python_files(self):
        """Yield (rel_posix_path, source_bytes) for every indexable .py file."""
...
def _resolve_calls(graph: Graph, indexers: List[_FileIndexer]) -> None:
...
class CodeGraph:
    ...
    def load_or_build(self, force: bool = False) -> Graph:
        ...
        if graph is None:
            return self.build()
        try:
            meta = json.loads((self.graph_dir / "meta.json").read_text(encoding="utf-8"))
        except (OSError, ValueError):
            return self.build()
    ...
    def _fmt_symbols(arg: str, g: Graph) -> str:
        ...
        if arg:
            ...
            infos = [i for i in infos
                    if a in i.name.lower() or a in i.qualified.lower()]
...
def _snapshot_mtimes(repo: Path) -> Dict[str, float]:
    """mtime snapshot of all indexable .py files, for freshness checks."""
    out: Dict[str, float] = {}
