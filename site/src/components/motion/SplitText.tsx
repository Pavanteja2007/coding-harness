 ...
 *
 * `highlight` renders exactly one word in --color-copper. The spec allows one
 * copper word in a heading, flat, and DESIGN.md §1.3 makes copper the "work is
 * happening" colour. On this site that word is "real", NOT "Verified": §1.4
 * reserves patina for anything verified, so colouring "Verified" copper would
...
export function SplitText({
  ...
  text: string;
  /** A word inside `text` to render in copper. Must appear verbatim in text. */
  highlight?: string;
  mode?: "chars" | "words";
  delayMs?: number;
  durationMs?: number;
  ...
  // take the copper token while still participating in the same stagger.
      ...
      <span aria-hidden="true">
        {units.map((u, i) => {
          ...
          return (
            <span
              key={i}
              className={u.hot ? "inline-block text-copper" : "inline-block"}
              style={{
